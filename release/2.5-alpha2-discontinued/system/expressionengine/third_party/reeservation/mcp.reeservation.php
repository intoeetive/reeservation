...
class Reeservation_mcp {
    ...
    function index()
    {
        ...
        if (!empty($this->settings['booking_entries']))
        {
            $this->EE->db->select('entry_id, channel_id, title');
            $this->EE->db->from('channel_titles');
            $this->EE->db->where_in('entry_id', explode(",",$this->settings['booking_entries']));
            $query = $this->EE->db->get();
            foreach ($query->result() as $obj)
            {
        ...
        foreach ($query->result() as $obj)
        {
           ...
           $vars['bookings'][$i]['date_from'] = $this->EE->localize->decode_date($date_format, $obj->date_from); 
           $vars['bookings'][$i]['date_to'] = ($obj->date_to!=$obj->date_from)?$this->EE->localize->decode_date($date_format, $obj->date_to):'';  
           $vars['bookings'][$i]['range'] = $obj->range_title;
        ...
        $vars['cp_notification'] = $this->_check_recent_upgrade($this->version);
    ...
    function edit()
    {
        ...
        $range_title = $this->EE->lang->line('not_defined');
        if ($query->row('range_id')!='')
        {
            $this->EE->db->select('range_title');
            $this->EE->db->from('reeservation_range_dates');
            $this->EE->db->where('range_id', $query->row('range_id'));
            $this->EE->db->limit(1);
        ...
        $vars['data'] = array(	
            ...
            'date_from'	=> "<strong>".$this->EE->localize->decode_date($date_format, $query->row('date_from'))."</strong>",
            'date_to'	=> ($query->row('date_to')!=$query->row('date_from'))?"<strong>".$this->EE->localize->decode_date($date_format, $query->row('date_to'))."</strong>":'',
            'range' 	=> $range_title,
            'places'	=> $query->row('places'),
            'comment'	=> nl2br($query->row('comment')),
            'booking_date'	=> $this->EE->localize->decode_date($date_format, $query->row('booking_date')),
        ...
        if ($query->row('moderator_id')!='')
        {
            ...
            $vars['data']['last_edit'] = $this->EE->localize->decode_date($date_format, $query->row('moderation_date'))." ".$this->EE->lang->line('by')." <a href=\"".BASE.AMP."D=cp&C=myaccount&id=".$query->row('moderator_id')."\">".$q->row('screen_name')."</a>";
    ...
    {
    	if (empty($this->settings['booking_channels']) || empty($this->settings['booking_entries']))
        {
        ...
        $vars['data'] = array(	
            'object'   	=> form_dropdown('entry_id', $entries),
    ...
    function submit_booking()
    {
        ...
        else
        {
            ...
            if ($this->EE->input->post('date_from')!='')
            {
                ...
                {
                    //wrong date format!
                    $errors[] = $this->EE->lang->line('wrond_date_format');
                }
            }
            else
            {
    ...
    function ranges()
    {
        ...
        $this->EE->db->select('range_id, range_title');
        $this->EE->db->from('reeservation_range_dates');
        $query = $this->EE->db->get();
        $vars['total_count'] = $query->num_rows();
        ...
        {
           $vars['ranges'][$i]['nr'] = $i;
           $vars['ranges'][$i]['title'] = "<a href=\"".BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=reeservation'.AMP.'method=range_edit'.AMP.'range_id='.$obj->range_id."\">".$obj->range_title."</a>";
           $vars['ranges'][$i]['delete_link'] = "<a class=\"reeservation_delete_warning\" href=\"".BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=reeservation'.AMP.'method=range_delete'.AMP.'range_id='.$obj->range_id."\">".$this->EE->lang->line('delete_range')."</a>";
           $i++;
        }
        ...
        $outputjs = '
			...
			$("<div id=\"reeservation_delete_warning\">'.$this->EE->lang->line('range_delete_warning').'</div>").dialog({
				autoOpen: false,
				resizable: false,
				title: "'.$this->EE->lang->line('confirm_deleting').'",
				modal: true,
				...
				"'.$this->EE->lang->line('delete_range').'": function() {
					location=draft_target;
				}
				}});
        ...
        $this->EE->cp->set_right_nav(
            array( 'add_time_range' => BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=reeservation'.AMP.'method=range_edit') 
        );
        
        $this->EE->cp->set_variable('cp_page_title', lang('reeservation_module_name'));
        
    	return $this->EE->load->view('ranges', $vars, TRUE);
    ...
    function range_edit()
    {
        ...
        $this->EE->cp->add_js_script('ui', 'datepicker'); 
        $this->EE->javascript->output(' $("input[name=date_from]").datepicker({ dateFormat: "'.$date_format_picker.'" }); '); 
        $this->EE->javascript->output(' $("input[name=date_to]").datepicker({ dateFormat: "'.$date_format_picker.'" }); '); 
        $this->EE->javascript->compile(); 
        ...
        //$setfor_a = array('d' => $this->EE->lang->line('one_day'), 'r' => $this->EE->lang->line('complete_range'));

        $range_id = intval($this->EE->input->get('range_id'));
        if ($range_id!=0)
        {
            $this->EE->db->select('*');
            $this->EE->db->from('reeservation_range_dates');
            $this->EE->db->where('range_id', $range_id);
            $query = $this->EE->db->get();
            
            $data['ranges']['range_title'] = form_input('range_title', $query->row('range_title')).form_hidden('range_id', $query->row('range_id'));
            $data['ranges']['date_from'] = form_input('date_from', $query->row('date_from'));
            $data['ranges']['date_to'] = form_input('date_to', $query->row('date_to'));
            $data['ranges']['dow_from'] = form_dropdown('dow_from', $dow_a, $query->row('dow_from'));
            $data['ranges']['dow_to'] = form_dropdown('dow_to', $dow_a, $query->row('dow_to'));
            $data['ranges']['time_from'] = form_dropdown('time_from', $time_a, $query->row('time_from')/(60*60));
            $data['ranges']['time_to'] = form_dropdown('time_to', $time_a, $query->row('time_to')/(60*60));
            //$data['ranges']['setfor'] = form_dropdown('setfor', $setfor_a, $query->row('setfor'));
        }
        else
        {

            $data['ranges']['range_title'] = form_input('range_title').form_hidden('range_id');
            $data['ranges']['date_from'] = form_input('date_from');
            $data['ranges']['date_to'] = form_input('date_to');
            $data['ranges']['dow_from'] = form_dropdown('dow_from', $dow_a);
            $data['ranges']['dow_to'] = form_dropdown('dow_to', $dow_a);
            $data['ranges']['time_from'] = form_dropdown('time_from', $time_a);
            $data['ranges']['time_to'] = form_dropdown('time_to', $time_a);
            //$data['ranges']['setfor'] = form_dropdown('setfor', $setfor_a);
            
        }
        
        $this->EE->cp->set_variable('cp_page_title', lang('reeservation_module_name'));
        
    	return $this->EE->load->view('range_edit', $data, TRUE);
    ...
    function range_save()
    {
        ...
        {        
            if (strpos($_POST['date_from'], "/")!==false)
            {
                //us date (%m/%d/%y)
            ...
            elseif (strpos($_POST['date_from'], "-")!==false)
            {
                //european date (%Y-%m-%d)
                $date_parts = explode("-", $_POST['date_from']);
                $m = $date_parts[1];
            ...
            $data['date_from'] = $this->EE->localize->convert_human_date_to_gmt($date_from_human);
        }

        
        if (!empty($_POST['date_to']))
        {        
            if (strpos($_POST['date_to'], "/")!==false)
            {
                //us date (%m/%d/%y)
            ...
            elseif (strpos($_POST['date_to'], "-")!==false)
            {
                //european date (%Y-%m-%d)
                $date_parts = explode("-", $_POST['date_to']);
                $m = $date_parts[1];
            ...
            $data['date_to'] = $this->EE->localize->convert_human_date_to_gmt($date_to_human);
        }


        if (!empty($_POST['dow_from']))
        { 
            $data['dow_from'] = $_POST['dow_from'];
        }
        if (!empty($_POST['dow_to']))
        { 
            $data['dow_to'] = $_POST['dow_to'];
        }
        if (!empty($_POST['dow_from']) && empty($_POST['dow_to']))
        { 
            $data['dow_to'] = $_POST['dow_from'];
        }
        
        if ((!empty($_POST['time_from']) && !empty($_POST['time_to'])) && ($_POST['time_from'] < $_POST['time_to']))
        { 
            $data['time_from'] = $_POST['time_from']*60*60;
            $data['time_to'] = $_POST['time_to']*60*60;
        }
        else
        if (!empty($_POST['time_from']) && !empty($_POST['time_to']))
        {
            $this->EE->session->set_flashdata('message_failure', $this->EE->lang->line('time_to_should_be_greater'));
            $this->EE->functions->redirect(BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=reeservation'.AMP.'method=ranges');
            return;
        }
        else
        if ((!empty($_POST['time_from']) && empty($_POST['time_to'])) || (empty($_POST['time_from']) && !empty($_POST['time_to'])))
        {
            $this->EE->session->set_flashdata('message_failure', $this->EE->lang->line('need_set_both_time'));
            $this->EE->functions->redirect(BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=reeservation'.AMP.'method=ranges');
            return;
        }

        $data['range_title'] = $this->EE->input->post('range_title');    
        //$data['setfor'] = $this->EE->input->post('setfor');             
        
        if (!empty($_POST['range_id']))
        {
            $this->EE->db->where('range_id', $this->EE->input->post('range_id'));
            $this->EE->db->update('reeservation_range_dates', $data);
        }
        else
        {
            $this->EE->db->insert('reeservation_range_dates', $data);
        }
    ...
    function range_delete()
    {

        if (!empty($_GET['range_id']))
        {
            $this->EE->db->where('range_id', $this->EE->input->get_post('range_id'));
            $this->EE->db->delete('reeservation_range_dates');
            if ($this->EE->db->affected_rows()>0)
            {
                $this->EE->session->set_flashdata('message_success', $this->EE->lang->line('range_deleted')); 
            }
            else
            {
                $this->EE->session->set_flashdata('message_failure', $this->EE->lang->line('no_range_to_delete'));  
            }
    ...
    function settings()
    {
        ...
        $channels = array();
        $_channels = $this->EE->api_channel_structure->get_channels($site_id);
        ...
        $vars['settings'] = array(	
            ...
            //'ranges_field'	=> form_dropdown('ranges_field', $fields, $this->settings['ranges_field']),
    ...
    function save_settings()
    {
        ...
        $settings['ranges_field'] = (isset($_POST['ranges_field']))?$this->EE->security->xss_clean($_POST['ranges_field']):'';
    ...
    function save_email_templates()
    {
        ...
        $templates = array('reeservation_created_admin_notification', 'reeservation_created_owner_notification', 'reeservation_created_user_notification', 'reeservation_edited_user_notification');
        //var_dump($_POST);
    ...
    function _check_recent_upgrade($version)
    {
        $this->EE->db->select('cp_notification');
        $this->EE->db->from('modules');
        $this->EE->db->where('module_name', 'Reeservation');  
        $this->EE->db->limit('1');      
        $query = $this->EE->db->get();
        $out = '';
        if ($query->row('cp_notification')!='' && $query->row('cp_notification') <= $this->version)
        {
            return $this->EE->lang->line('cp_notification_'.$version);
        }
        return false;
    }
